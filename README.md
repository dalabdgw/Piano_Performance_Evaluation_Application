...
| 단계 | 핵심 기능 | 한줄 설명 |
|------|-----------|-----------|
| **1. 업로드** | 음원(mp3)·악보 이미지를 업로드 → 자동으로 10 초 세그먼트로 분할 | 관리자가 S3 에 업로드 후 메타데이터를 RDS 저장 |
| **2. 곡 탐색** | 홈(페이지 네비게이션)·검색 창 | 곡명 기준 필터·검색 → 원하는 곡·세그먼트 선택 |
| **3. 익명 평가** | 연주자 정보 없이 **곡명·세그먼트**만 노출 | 평가 공정성 확보, 편향 최소화 |
| **4. 악보·오디오 감상** | 10 초 세그먼트 + 해당 악보 이미지 동시 제공 | 집중 감상 후 점수 입력 |
| **5. 15 항목 라벨링** | 톤, 레가토, 해석 … 페달 등 15가지 항목에 1-10점 매기기 | 점수와 코멘트가 `review` 테이블에 실시간 저장 |
| **6. 결과 추출** | SQL 또는 CSV 내보내기 | 연구·통계·모델 학습용 라벨 데이터 즉시 활용 |

> **한줄 요약 :**  
> *“업로드 → 10초 단위 분할 → 악보·오디오 동시 재생 → 15항목 익명 평가 → DB 저장 → CSV 추출”* 까지를 **웹 한 곳**에서 끝낼 수 있는 경량형 파이프라인입니다.
